...
import org.scoula.basic.command.Command;
import org.scoula.basic.controller.HomeController;
import org.scoula.basic.controller.TodoController;
...
// 프론트 컨트롤러 서블릿: 모든 요청을 받아 알맞은 컨트롤러로 전달하는 중앙관리 서블릿
@WebServlet("/") // 필터는 @WebServlet("/*") 더 먼저 실행
public class FrontControllerServlet extends HttpServlet {
    // 요청 URL을 키로 매핑 테이블 처럼 관리하는 구조(시간 복잡도 O(1))
    ...
    // 서블릿 초기화 시 호출되는 메서드
    public void init() {
        // get 요청별 어떤 메서드를 호출해야하는지 hashmap에 모아보자
        // hashmap의 key는 요청주소, value는 호출해야하는 메서드 이름.
        getMap = new HashMap<>();
        postMap = new HashMap<>();
    ...
    public void execute(Command command, HttpServletRequest req, HttpServletResponse resp) throws IOException, ServletException {
        ...
        if (viewName.startsWith("redirect:")) {
            resp.sendRedirect(viewName.substring("redirct:".length()));
        } else {
            // 그외는 JSP 포워드 처리
    ...
    // Post 요청도 GET처럼 처리(일부 방식에서는 분리 권장됨) -> doGet 오버라이딩
    @Override
    ...
    private Command getCommand(HttpServletRequest req) {
        ...
        // GEt 이면 GetHash에서 어떤 함수를 불러야하는지 알아오고,
